#== RULES general_competitive_low
      ...
      - id: cl_takeout_X_agreed
        call: X
        priority: 36.5
        when: { their_last_bid_suit: true, side_has_acted: false, standing_bid_level: [2], we_hold_contract: false }
        requires:
          hcp: [11, 40]
          evals: { standing_suit_length: [0, 2], weakest_unbid_length: [3, 13] }
        shows: "takeout double of the suit they have agreed: at most a doubleton in THAT suit and three cards in each unbid suit"
        establishes: { forcing: one_round }
      - id: cl_support_X_CH
        call: X
        priority: 36.4
        when: { my_suit: C, partner_last_suit: H, standing_bid_level: [1], their_last_bid_suit: true, we_hold_contract: false }
        requires: { suits: { H: [3, 3] }, hcp: [12, 21] }
        shows: "support double: exactly three hearts for partner's one-level response"
      ...
      - id: cl_support_X_CS
        call: X
        priority: 36.4
        when: { my_suit: C, partner_last_suit: S, standing_bid_level: [1], their_last_bid_suit: true, we_hold_contract: false }
        requires: { suits: { S: [3, 3] }, hcp: [12, 21] }
        shows: "support double: exactly three spades for partner's one-level response"
        establishes: { forcing: non_forcing }
        alertable: true
        convention: support_double
      - id: cl_support_X_DH
        call: X
        priority: 36.4
        when: { my_suit: D, partner_last_suit: H, standing_bid_level: [1], their_last_bid_suit: true, we_hold_contract: false }
        requires: { suits: { H: [3, 3] }, hcp: [12, 21] }
        shows: "support double: exactly three hearts for partner's one-level response"
        establishes: { forcing: non_forcing }
        alertable: true
      ...
      - id: cl_support_X_DS
        call: X
        priority: 36.4
        when: { my_suit: D, partner_last_suit: S, standing_bid_level: [1], their_last_bid_suit: true, we_hold_contract: false }
        requires: { suits: { S: [3, 3] }, hcp: [12, 21] }
        shows: "support double: exactly three spades for partner's one-level response"
        establishes: { forcing: non_forcing }
        alertable: true
      ...
      - id: cl_reopen_X2
        ...
        shows: "a SECOND takeout double: 19+, still nothing to say but takeout"
        establishes: { forcing: one_round }
      - id: cl_penalty_X_over_nt
        call: X
        priority: 35.2
        when: { side_has_acted: true, i_have_acted: false, standing_bid_strain: [NT],
                standing_bid_level: [1], we_hold_contract: false }
        requires:
          hcp: [9, 40]
          evals: { their_bidders: [1, 1] }
        shows: "penalty double of their natural 1NT overcall: partner opened, so our side holds the balance of strength"
        establishes: { forcing: non_forcing }
      - id: cl_reopen_X
        call: X
        priority: 34.8
        ...
        shows: "second-round takeout double: 16+, short in their suit, our side already in the auction"
        establishes: { forcing: one_round }
      # ---------------------------------------------------------------
      # The two-level negative double showing BOTH majors.  Level-gated to
      # [2], so it cannot reach `cl_negative_X1`.
      # ---------------------------------------------------------------
      ...
      # ---------------------------------------------------------------
      # FAMILY 10 - route hands INTO the competitive raise.
      # `cl_raise_S2` / `cl_raise_H2` measure +3.12 / +2.10 a table.
      # ---------------------------------------------------------------
      - id: cl_raise_pref_H2
        call: 2H
        priority: 33.4
...
#== RULES general_competitive_high
      ...
      - id: ch_sac_X_trumps
        ...
        when: { their_last_bid_suit: true, we_bid_last: false, side_has_acted: true,
                standing_bid_level: [5, 6], we_hold_contract: false }
        requires:
        ...
        establishes: { forcing: non_forcing }
      - id: ch_takeout_X_shape
        call: X
        priority: 37.0
        when: { their_last_bid_suit: true, side_has_acted: false, we_hold_contract: false }
        requires:
          evals: { longest_suit_length: [0, 5] }
          any_of:
            - hcp: [12, 40]
        ...
        shows: "takeout double of their preempt priced in SHAPE: 12+ with at most a doubleton in their suit, 10+ with a singleton, and no long suit of my own to bid"
        establishes: { forcing: one_round }
      - id: ch_takeout_X_acted
        ...
        requires:
          hcp: [13, 40]
          evals: { standing_suit_length: [0, 2], quick_tricks: [2, 12], longest_suit_length: [0, 6] }
        ...
        establishes: { forcing: one_round }
      # The push over their pre-emptive leap to game.
...
#== RULES general_balancing_low
      ...
      - id: ballow_reopen_X_opener
        ...
        requires:
          hcp: [13, 15]
          evals: { max_their_suit_length: [0, 2], longest_suit_length: [0, 5] }
        shows: "opener reopens with a double: opening values, short in their suit - partner may pass it for penalties"
        establishes: { forcing: one_round }
      - id: ballow_reopen_X2_shape
        call: X
        priority: 40.5
        when: { their_last_bid_suit: true, side_has_acted: true, we_bid_last: false,
                my_last_call_was_double: true, standing_bid_level: [2], we_hold_contract: false }
        requires:
          hcp: [15, 18]
      ...
      - id: ballow_reopen_X_shape
        ...
        establishes: { forcing: one_round }
      # ---------------------------------------------------------------
      # THE BRAKE.  `ballow_raise_$M4` (32) bids game on a shaded 25-count
      # over an EIGHT-card fit.  These two rungs sit just above it and say
      # eight trumps is not a game raise.  The two-level and three-level
      # siblings carry different priorities so a decision is never handed
      # to arbitration by a tie.
      # ---------------------------------------------------------------
...
#== RULES general_balancing_high
      ...
      - id: balhigh_penalty_X
        ...
        shows: "penalty double in the balancing seat: four-plus of their trumps behind the bidder and three quick tricks - they have preempted into my trump stack"
        establishes: { forcing: non_forcing }
      ...
      - id: balhigh_X_shape
        ...
        requires:
          ...
          any_of:
            ...
            - hcp: [10, 40]
              evals: { standing_suit_length: [0, 1] }
        shows: "balancing double of their preempt priced in SHAPE: 12+ with at most a doubleton in their suit, 10+ with a singleton, no long suit of my own to bid"
        establishes: { forcing: one_round }
      - id: balhigh_rebid_H5
        call: 5H
        priority: 33.0
